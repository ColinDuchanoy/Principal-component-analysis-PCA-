# -*- coding: utf-8 -*-
"""
Created on Fri Jan 14 09:50:54 2022
...
def cor(X, Y):  # Fonction de Corélation entre deux matrices de même taille
    ...
    -------
    Cori : Matrice
        Renvoie la correlation entre Xi et Y 
    """
    Cori = []
...
def correlationdirprinc(R, k):  # Ensemble des corélation de Yk et Xi
    ...
    -------
    Cori : Matrice
        Renvoie la matrice des corélation entre Xi et Yk 
    """
    X = centre_red(R)
    Y = approx(X, k)
    Cori = []
...
def ACP2D(R, labelsligne, labelscolonne) :
    ...
    fig1, (ax1,ax2) = plt.subplots(1,2) #séparation  de la fenetre en deux pour afficher deux graphiques
    plt.suptitle('Figure 1 – Valeurs des variances et pourcentages explicatifs de celles-ci pour les composantes principales',fontsize=12) #Titre général de la fenêtre
    # Tracé de la variance des composantes principales
    
    X = centre_red(R)               #La matrice  des données est centrée et réduite 
    u, s, vt = np.linalg.svd(X)     #Décomposition SVD de la matrice X centrée réduite
    v = vt.T                        #Transposition du vecteur vt provenant de la décompositon SVD. v Vecteur 
    
    m,k = np.shape(X)                 #On récupère les dimensions de X la matrice centrée réduite sur laquelle on travaille (Même dimension que R)
    Yk = np.zeros(shape=(k,2), dtype=object) #On crée une matrice Yk de 2 colonnes. Elle nous servira à afficher les variances des composantes
    for i in range(k):                          #On remplit cette matrice Yk
        vk = v[:, i].reshape(len(v), 1)
        Yk[i,0] = '$Y_{}$'.format(i+1)            #La première colonne contient le label Yk
        Yk[i,1] = Variance(X@vk)                  #La seconde colonne contient la valeur de la variance correspondante
    Yksum = 0
    ...
    Ykvalues = list([]) #Création d'une nouvelle liste qui ne contient que les valeurs des différentes variances pour tracer le graphique en barres 
    ...
    for index, value in enumerate(labelsYkvalues) :              #On place les valeurs de chacune des barres au dessus de celles-ci
    ...
    ax2.pie(Ykpercent,labels = ind
            ,autopct = lambda Ykpercent : str(round(Ykpercent, 1)) + '%',shadow = True) # "autopct" permet d'ajouter sur chacune des parts
    ax2.set_title('Participation à la variance totale')                                 # la proportion en % qu'elle représente 
    ...
    plt.suptitle('Figure 2 – Représentation des données dans le plan (Y1, Y2) et le cercle de corrélations dans ce même plan',fontsize=12)
    ...
    #Création  d'un  dictionnaire contenant chacun des labels ligne différents et assignation d'une couleur aléatoire.
    Dico = {}
    legende= []
    ...
    #Tracé des points
    for i in range(m) : 
        ax3.scatter(App[i,0], App[i,1],s = 10*2**1,color = '#' + Dico[lbl[i]],label="test")
    
    #Création de la légende en se servant du dictionnaire
    patchList = []
    for key in Dico:
        data_key = mpatches.Patch(color='#' + Dico[key], label=key)
        patchList.append(data_key)
    ax3.legend(handles=patchList,loc="upper left")   
    ...
    draw_circle = plt.Circle((0,0), 1,fill = False,ls = '--',lw = '1.5',color = 'royalblue')
    ax4.set_aspect(1)
    ...
    for i in range(np.shape(R)[1]) :
        ax4.arrow(0,0,Corx[i],Cory[i],width = 0.02,length_includes_head = True,color = 'royalblue')
        ax4.annotate(text=labelscolonne[i], xy = (Corx[i],Cory[i]),  fontsize=12) 
...
def ACP3D(R, labelsligne, labelscolonne) :
    ...
    X = centre_red(R)               #La matrice  des données est centrée et réduite 
    u, s, vt = np.linalg.svd(X)     #Décomposition SVD de la matrice X cnetrée réduite
    ...
    #Utilisation de la méthode découverte en Ma 313 pour colorier chacun des points du nuage selon la valeur de leur labelsligne
    #Création d'un dictionnaire contenant chacun des labels ligne différentes et assignant une couleur aléatoire à ces labels.
    Dico = {}
    n = len(set(labelsligne))
    lbl = labelsligne
    for i in labelsligne :
    ...
    #Tracé du label des points si cela ne surcharge pas le graphique
    if len(Dico)<=5:
        for i, txt in enumerate(lbl) :
            ax3.text(App[i,0],App[i,1],App[i,2],  txt, size=8, color = '#' + Dico[lbl[i]])  
...
def ACP(R, labelsligne, labelscolonne, k = 0, epsilon = 10**-1):
    ...
    Returns
    -------
    regle de Kaiser à la fin du code
    None.
    """
    ...
    # Mise en page de la fenetre matplotlib
    fig1, (ax1,ax2) = plt.subplots(1,2) #séparation  de la fentre en deux pour afficher deux graphiques
    plt.suptitle('Figure 1 – Valeurs des variances et pourcentage explicatif de celles-ci pour les composantes principales',fontsize=12) #Titre général de la fenêtre
    # Tracé de la Variance des composantes principales
    ...
    m,n = np.shape(X)                 #On récupère les dimensions de X la matrice centrée réduite sur laquelle on travaille (Même dimension que R)
    Yk = np.zeros(shape = (n,2), dtype = object)    #On crée une matrice Yk de 2 colonnes. Elle nous servira à afficher les variances des composantes
...
def Kaiser(E,epsilon=10**-1): 
    Y=centre_red(E)    #On centre reduite la matrice E
    U,S,VT=np.linalg.svd(Y)   #On effectue la decomposition SVD

    
    for k in range(len(S)):               #On parcourt la matrice S qui renvoie les valeurs singulieres
        if (S[k])**2>1-epsilon:           #On ecrit les condtions à respecter 
            if (S[k+1])**2<1-epsilon:   
                
                q=k                     #On renvoie la valeur de k pour laquelle la condtion est respecté
